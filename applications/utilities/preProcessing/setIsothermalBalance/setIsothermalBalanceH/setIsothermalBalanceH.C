...
// * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * //
    ...
    for(label iter = 0; iter < maxIters && !converged; iter++)
    {
        ...
        (
            fvm::laplacian(Cp*T, lnExner)
          + fvc::div(Cp*linearInterpolate(T)*
                     Hodge.ddirToFluxCorr(fvc::snGrad(lnExner)*Hodge.magd()))
             == fvc::div(Hodge.ddirToFlux(gd))
        );
    ...
    for(label iter = 0; iter < maxIters && !converged; iter++)
    {
        ...
        + fvc::div(Cp*thetaf*Hodge.ddirToFluxCorr(fvc::snGrad(Exner)*Hodge.magd()))
             == fvc::div(Hodge.ddirToFlux(gd))
        );
