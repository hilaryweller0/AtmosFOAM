...
Description
    Calculates distance from the ground based on the orography specification
...
#   include "createTime.H"
    ...
    (
        IOobject
        (
            "mountainDict", mesh.time().system(), mesh,
            IOobject::MUST_READ, IOobject::NO_WRITE
        )
    );
    const autoPtr<mountain> m = mountain::New(mountainDict.subDict("mountain"));
    ...
    // Ground distance that varys in space
    volScalarField groundDistance
    (
        IOobject("groundDistance", mesh.time().constant(), mesh),
        mesh.C().component(2)
    );
    
    forAll(groundDistance, cellI)
    {
        // Cell centre
        const point& C = mesh.C()[cellI];

        groundDistance[cellI] -= (m->heightAt(C)).value();
    }

    groundDistance.write();
}
