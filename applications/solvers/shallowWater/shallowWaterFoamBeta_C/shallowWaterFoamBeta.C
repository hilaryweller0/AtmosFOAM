...
#       include "CourantNo.H"
        ...
        for (int corr=0; corr<nCorr; corr++)
        {
            ...
            // without Coriolis (so as not to create flow over boundaries)
            ...
            // Add explicit component of the flux that can create flow
            // over boundaries
            if (rotating) flux = fluxU - dt*offCentre*hf*(fSfxk & Uf);
            else flux = fluxU;
            
            // Helmholtz (pressure) equation
            fvScalarMatrix hEqn
            (
                fvm::ddt(h)
              + fvc::div((1-offCentre)*flux.oldTime() + offCentre*flux)
              - fvm::laplacian(sqr(offCentre)*g*hf*dt, h)
            );
