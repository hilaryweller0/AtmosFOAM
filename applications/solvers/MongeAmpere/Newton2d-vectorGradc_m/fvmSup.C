...
)
{
    ...
    DimensionedField<scalar, volMesh> suspPos = susp;
    DimensionedField<scalar, volMesh> suspNeg = susp*0;
    forAll(susp, cellI)
    {
        if (sign(susp[cellI]) != sign(fvm.diag()[cellI]))
        {
            suspPos[cellI] = 0;
            suspNeg[cellI] = susp[cellI];
        }
    }

    fvm.diag() += mesh.V()*max(suspPos.field(), scalar(0));

    fvm.source() -= mesh.V()*min(suspNeg.field(), scalar(0))
        *vf.internalField();
