...
def schaerProfileTemperature(x,z,dist_center_x,dist_center_z,half_width,theta,w,t,L):
    g = 9.81
    c_p = 1004.
    theta0 = 300.
    b = g/(c_p*theta0)
    ...
    if r0 < 1:
        ...
        theta_dash = math.atan2(z0,x0) - np.pi/2
        T = 2*np.pi/w
        expon = np.exp( -theta0/L*( (1-b*z0)*T/2.) )
        factor = 1.
        ...
        if theta_dash >= 0.:
            ...
            if int(cycle_phase2)%2 == 0:
                for i in range(int(cycle_phase)):
                    #Special case for first half-cycle.
                    if i == 0:
                        factor *= np.exp( -theta0/L*( (1-b*z0)*(T/2.+theta_dash/w) - b*r/w*np.cos(theta_dash + np.pi/2.) ) )
                    else:
                        factor *= expon
                    factor = 1 - (1-factor)*expon
                #Special case for first half-cycle.
                if int(cycle_phase) == 0:
                    factor *= np.exp( -theta0/L*( (1-b*z0)*(t%T) + b*r/w*( np.cos(w*((t+theta_dash/w)%T) + np.pi/2.) - np.cos(theta_dash + np.pi/2.) ) ) )
                else:
                    factor *= np.exp( -theta0/L*( (1-b*z0)*((t+theta_dash/w)%T) + b*r/w*np.cos(w*((t+theta_dash/w)%T) + np.pi/2.) ) )
            #If in second half of cycle.
            else:
                for i in range(int(cycle_phase)+1):
                    if i == 0:
                    ...
                    if i != int(cycle_phase):
                        factor = 1 - (1-factor)*expon
                factor = 1 - (1-factor) * np.exp( -theta0/L*( (1-b*z0)*((t+theta_dash/w)%(T/2)) + b*r/w*np.cos(w*((t+theta_dash/w)%(T/2)) + np.pi/2.) ) )
...
def readField(filename,size):
    ...
    try:
        ...
        print "No Field Detected. Assuming uniform 0 Field."
