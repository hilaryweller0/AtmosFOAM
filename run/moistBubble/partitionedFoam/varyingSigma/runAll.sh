...
for time in 0 ?? [1-3]??; do
    sumFields $time stableThetaDiff $time stable.theta constant thetaRef -scale1 -1
    gmtFoam -time $time stableThetaDiff
    sumFields $time buoyantThetaDiff $time buoyant.theta constant thetaRef -scale1 -1
...
# Plotting total fields
time=182
gmtFoam -time $time buoyantWaterLiquidFrac
gmtFoam -time $time stableWaterLiquidFrac 
gmtFoam -time $time stableTheta
gmtFoam -time $time buoyantTheta
gmtFoam -time $time stableWaterVapourRho
gmtFoam -time $time buoyantWaterVapourRho
