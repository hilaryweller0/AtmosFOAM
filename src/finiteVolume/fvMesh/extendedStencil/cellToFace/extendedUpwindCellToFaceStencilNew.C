...
void Foam::extendedUpwindCellToFaceStencilNew::selectOppositeFaces
(
...
) const
{
    ...
    // (ownder first for linearCorr, upwind cell first for linearCorr=false)
    if (stencilHasNeighbour)
    {
        ...
        if (n != transportedStencil.size())
        {
            FatalErrorIn
            (
                "extendedUpwindCellToFaceStencilNew::transportStencil(..)"
...
Foam::extendedUpwindCellToFaceStencilNew::extendedUpwindCellToFaceStencilNew
(
