...
void Foam::primitiveMesh::calcCellCentresAndVols() const
{
    ...
    // Make the constraint type boundary faces orthogonal
    vectorField& fCtrs = *faceCentresPtr_;
    for(label facei = nInternalFaces(); facei < nFaces(); facei++)
    {
        label own = faceOwner()[facei];
        vector Sfhat = faceAreas()[facei]/mag(faceAreas()[facei]);
        fCtrs[facei] = cellCtrs[own]
                     + ((fCtrs[facei] - cellCtrs[own]) & Sfhat)*Sfhat;
    }
