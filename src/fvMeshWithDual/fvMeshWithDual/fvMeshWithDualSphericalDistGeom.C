...
void Foam::fvMeshWithDual::calcSphericalDistGeom() const
{
    ...
    forAll(fCtr, faci)
    {
        ...
        if (!faceOnSphere)
        {
            ...
            // Check that both rhats have been found
            if (rhati == 0)
            {
                FatalErrorIn("fvMeshWithDual::calcSphericalDistGeom")
                    << " face " << faci << " is vertical and so should have "
                    << "only two directions but it has unit vertices "
                    << nl << p[0] << nl << p[1] << nl << p[2] << nl << p[3]
                    << exit(FatalError);
            }
            
            // rhat for the face centre
            const vector rhat = readFaceCentres_ ? unitVector(fCtr[faci])
                                            : unitVector(intersections()[faci]);
        ...
        else // face on the sphere
        {
            ...
            for(label ip = 0; ip < p.size(); ip++)
            {
                ...
                scalar a = sphTriDistAngle(fCentre, p[ip], p[iprev]);
                omega += a;
                sumR += r*a;
                iprev = ip;
            }
            ...
            fCtr[faci] = rhat*sumR;
            fArea[faci] = sqr(sumR)*rhat*sign(fArea[faci] & rhat)
                        *patchFaceArea()[faceToPatchFace(faci)];
        }
    }
    ...
    // set deltaCoeffs as inverse of great circle distances
    surfaceScalarField dCoeffs = deltaCoeffs();
    forAll(dCoeffs, facei)
    {
        dCoeffs[facei] = 1./sphDist(C()[neighbour()[facei]], C()[owner()[facei]]);
    }
    surfaceInterpolation::overrideDeltaCoeffs(dCoeffs);
}
